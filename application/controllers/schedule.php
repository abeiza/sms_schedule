...
class Schedule extends CI_Controller{
							...
							if(!preg_match('/^[0-9]{1,}$/',$text)){
								...
								$this->model_app->getInsertMy('outbox',$dataoutbox2);
									...
									if($range <= 2 and $range > -1){
										//if(($range <= 2 and $range > 0)or($range > -1 and date('H') > 14)){
	...
	function filter_full_format(){
		$query = $this->model_app->getQueryMy("select * from Final_Inbox WHERE Processed='loading'");
		foreach($query->result() as $db){
			...
			//kode baru dengan kode ba
				...
				if(count($product) === count($qty) and count($fail) === 0){
					...
					//echo 'jumlahnya : '.$count_stts['1'];
					$query_outlet = $this->model_app->getQueryMy("SELECT ID_OUTLET FROM Ms_OUTLET WHERE ID_OUTLET='".$outlet."'");
					$query_ba = $this->model_app->getQueryMy("SELECT ID_BA FROM Ms_BA WHERE ID_BA='".$ba."' and STATUS=1");
					$query_true = $this->model_app->getQueryMy("SELECT ID_BA, ID_OUTLET FROM Ms_OUTLET WHERE ID_BA='".$ba."' and ID_OUTLET='".$outlet."'");
					
					$hari = substr($db->TextSMS,0,2);
					$bulan = substr($db->TextSMS,2,2);
					$tahun = substr($db->TextSMS,4,2);
					
					if ($count_stts['1'] == count($product) and $query_outlet->num_rows() > 0 and $query_ba->num_rows() > 0){
						if($query_true->num_rows() > 0){
							$data_f['ID_BA'] = $ba;
							$data_f['Processed'] = 'valid';
							$data_f['ReplySMS'] = 'Terimakasih, sms anda akan segera kami proses';
							$query_update = $this->model_app->getUpdateMy('Final_Inbox',$data_f,'ObjectID',$db->ObjectID);
							
							//$absen['ReceiveDt'] = $db->ReceiveDt;
							//$absen['tahun'] = $tahun;
							//$absen['bulan'] = $bulan;
							//$absen['hari'] = $hari;
							//$absen['ID_BA'] = $ba;
							//$absen['Processed'] = 'valid';
							//$dat = $ba;
							//if($dat != ''){
							...
							//	$absen['ID_OUTLET'] = null;
							//}
							/*$absen['Freq_SMS'] = $db->FreqSMS;
							$absen['ReplySMS'] = 'Terimakasih, sms anda akan segera kami proses';
							$absen['TextSMS'] = $db->TextSMS;
							$this->model_app->getInsertSrv('BA_Freq_Absen',$absen);*/
							
							$data_a['ReplySMS'] = 'Terimakasih, sms anda akan segera kami proses';
							$query_update2 = $this->model_app->getUpdateMy('new_inbox',$data_a,'object_id',$db->RefObjectID);
							//echo "SUKSES SIMPAN VALID";
							$dataout3['DestinationNumber'] = $sender;
							$dataout3['TextDecoded'] = 'Terimakasih, sms anda akan segera kami proses';
							$dataout3['CreatorID'] = 'Gammu';
							$this->model_app->getInsertMy('outbox',$dataout3);
						}else{
							$data_f['ID_BA'] = $ba;
							$data_f['Processed'] = 'invalid';
							$data_f['ReplySMS'] = 'Maaf, Kode BA yang digunakan tidak sesuai kode toko. Mohon periksa kembali sms anda.';
							$query_update = $this->model_app->getUpdateMy('Final_Inbox',$data_f,'ObjectID',$db->ObjectID);
							...
							//}
							/*$absen['Freq_SMS'] = $db->FreqSMS;
							$absen['ReplySMS'] = 'Maaf, Kode BA yang digunakan tidak sesuai kode toko. Mohon periksa kembali sms anda.';
							$absen['TextSMS'] = $db->TextSMS;
							$this->model_app->getInsertSrv('BA_Freq_Absen',$absen);*/
							
							$data_a['ReplySMS'] = 'Maaf, Kode BA yang digunakan tidak sesuai kode toko. Mohon periksa kembali sms anda.';
							$query_update2 = $this->model_app->getUpdateMy('new_inbox',$data_a,'object_id',$db->RefObjectID);
							//echo "SUKSES SIMPAN VALID";
							$dataout3['DestinationNumber'] = $sender;
							$dataout3['TextDecoded'] = 'Maaf, Kode BA yang digunakan tidak sesuai kode toko. Mohon periksa kembali sms anda.';
							$dataout3['CreatorID'] = 'Gammu';
							$this->model_app->getInsertMy('outbox',$dataout3);
						}
						...
						$data_f['ID_BA'] = "";
						$data_f['Processed'] = 'invalid';
						$data_f['ReplySMS'] = 'Maaf, Kode BA tidak terdaftar. Mohon anda periksa kembali kode BA';
						$query_update = $this->model_app->getUpdateMy('Final_Inbox',$data_f,'ObjectID',$db->ObjectID);
						...
						/*$absen['Freq_SMS'] = $db->FreqSMS;
						$absen['ReplySMS'] = 'Maaf, Kode BA tidak terdaftar. Mohon anda periksa kembali kode BA';
						$absen['TextSMS'] = $db->TextSMS;
						$this->model_app->getInsertSrv('BA_Freq_Absen',$absen);*/
						
						$data_a['ReplySMS'] = 'Maaf, Kode BA tidak terdaftar. Mohon anda periksa kembali kode BA';
						$query_update2 = $this->model_app->getUpdateMy('new_inbox',$data_a,'object_id',$db->RefObjectID);
						//echo "gagal";
						$dataout2['DestinationNumber'] = $sender;
						$dataout2['TextDecoded'] = 'Maaf, Kode BA tidak terdaftar. Mohon anda periksa kembali kode BA';
						$dataout2['CreatorID'] = 'Gammu';
						$this->model_app->getInsertMy('outbox',$dataout2);
					}else if($query_outlet->num_rows() == 0){
						//foreach($query_ba1->result() as $bad){
						...
						$data_f['ID_BA'] = $ba;
						$data_f['Processed'] = 'invalid';
						$data_f['ReplySMS'] = 'Maaf, Kode Toko tidak terdaftar. Mohon anda periksa kembali kode toko';
						$query_update = $this->model_app->getUpdateMy('Final_Inbox',$data_f,'ObjectID',$db->ObjectID);
						...
						//}
						/*$absen['Freq_SMS'] = $db->FreqSMS;
						$absen['ReplySMS'] = 'Maaf, Kode Toko tidak terdaftar. Mohon anda periksa kembali kode toko';
						$absen['TextSMS'] = $db->TextSMS;
						$this->model_app->getInsertSrv('BA_Freq_Absen',$absen);*/
						
						$data_a['ReplySMS'] = 'Maaf, Kode Toko tidak terdaftar. Mohon anda periksa kembali kode toko';
						$query_update2 = $this->model_app->getUpdateMy('new_inbox',$data_a,'object_id',$db->RefObjectID);
						//echo "gagal";
						$dataout2['DestinationNumber'] = $sender;
						$dataout2['TextDecoded'] = 'Maaf, Kode Toko tidak terdaftar. Mohon anda periksa kembali kode toko';
						$dataout2['CreatorID'] = 'Gammu';
						$this->model_app->getInsertMy('outbox',$dataout2);
					}else if($count_stts['1'] != count($product)){
						for($i = 0; $i <= count($product_not);$i++){
						...
						$data_f['ID_BA'] = $ba;
						$data_f['Processed'] = 'invalid';
						$data_f['ReplySMS'] = 'Maaf, Ada Kode Barang yang tidak terdaftar. Kode Barang '.$prd.' Tidak Terdaftar. Mohon anda periksa kembali Kode Barang';
						$query_update = $this->model_app->getUpdateMy('Final_Inbox',$data_f,'ObjectID',$db->ObjectID);
						...
						//}
						/*$absen['Freq_SMS'] = $db->FreqSMS;
						$absen['ReplySMS'] = 'Maaf, Ada Kode Barang yang tidak terdaftar. Kode Barang '.$prd.' Tidak Terdaftar. Mohon anda periksa kembali Kode Barang';
						$absen['TextSMS'] = $db->TextSMS;
						$this->model_app->getInsertSrv('BA_Freq_Absen',$absen);*/
						
						$data_a['ReplySMS'] = 'Maaf, Ada Kode Barang yang tidak terdaftar. Kode Barang '.$prd.' Tidak Terdaftar. Mohon anda periksa kembali Kode Barang';
						$query_update2 = $this->model_app->getUpdateMy('new_inbox',$data_a,'object_id',$db->RefObjectID);
						//echo "gagal";
						$dataout2['DestinationNumber'] = $sender;
						$dataout2['TextDecoded'] = 'Maaf, Ada Kode Barang yang tidak terdaftar. Kode Barang '.$prd.' Tidak Terdaftar. Mohon anda periksa kembali Kode Barang';
						$dataout2['CreatorID'] = 'Gammu';
						$this->model_app->getInsertMy('outbox',$dataout2);
						$prd = "";
					}
					...
					if(count($fail) !== 0 or count($fail_product) !== 0 or count($fail_qty) !== 0)
					{
						if(count($fail) !== 0){
							...
							//}
							/*$absen['Freq_SMS'] = $db->FreqSMS;
							$absen['ReplySMS'] = 'Mohon maaf, Format yang anda gunakan salah. Format benar : tglsales(ddmmyy) <spasi> kodeBA <spasi> kodetoko#kodeproduk <spasi> qty#kodeproduk <spasi> qty';
							$absen['TextSMS'] = $db->TextSMS;
							$this->model_app->getInsertSrv('BA_Freq_Absen',$absen);*/
				...
				$data_f['ID_BA'] = "";
				$data_f['Processed'] = 'invalid';
				$data_f['ReplySMS'] = 'Format tidak sesuai. Mohon diperiksa kembali sms anda.';
				$query_update = $this->model_app->getUpdateMy('Final_Inbox',$data_f,'ObjectID',$db->ObjectID);
				
				$data_a['ReplySMS'] = 'Format tidak sesuai. Mohon diperiksa kembali sms anda.';
				$query_update2 = $this->model_app->getUpdateMy('new_inbox',$data_a,'object_id',$db->RefObjectID);
											
				$dat10['DestinationNumber'] = $db->SenderNumber;
				$dat10['TextDecoded'] = 'Format tidak sesuai. Mohon diperiksa kembali sms anda.';
	...
	function new_header_detail(){
		$query_new_header = $this->model_app->getQuerySrv("SELECT TOP 100 * FROM Final_Inbox WHERE Processed='valid'");
